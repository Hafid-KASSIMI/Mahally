 ...
 * @author Sicut
 */
public class CandsListPDFFile extends BasePDFFile {
    
    private CandsListPage metrics;
    private final DecimalFormat INTEGER;
    private int room, roomCands, i;
    private PDPage pg;
    
    public CandsListPDFFile() {
        super();
        metrics = null;
        INTEGER = new DecimalFormat("00");
        INTEGER.setDecimalFormatSymbols(DecimalFormatSymbols.getInstance(Locale.FRANCE));
        tpl = "/localexam/template/resources/LIST_TEMPLATE.pdf";
    }
    
    public Boolean generate() {
        reset();
        if ( metrics == null )
            metrics = new CandsListPage();    
        room = -1;
        i = 0;
        ...
        return ( doc.getPages().getCount() > 0 ) ? 
            save(PREF_BUNDLE.get("OUTPUT_DIR") + generateFileName("List-", "pdf")) : false;
    }
